...
进入 **智能体商店** → 点击页面右上角 **上架应用**。当前新版界面没有“新建商品”按钮；旧文档中的“创建商品”与当前界面的“上架应用”是同一个后台动作。

**上架应用**只创建目录应用草稿和首个客户部署，并不会立即让用户看到。管理员仍需完成“部署验证 → 开启执行 → 发布”，用户目录才会出现该应用。
...
同一目录应用需要服务多个客户时，点击应用卡片中的 **添加客户部署**，选择新的客户和该客户自己的 Customer App。不要复制目录元数据；每个部署独立验证、授权、启停和审计。

## 11. 配置使用授权（Entitlement）

上架应用时授权列表为空，服务端会自动创建“整个部署客户可用”的默认授权。需要更细粒度限制时，在部署卡片点击 **管理部署** → **添加授权**。
...
6. 点击目录应用级 **发布**。
7. 发布后目录应用状态应为 `published`，已验证部署状态应为 `active`。

如果先发布后开启执行也能保存，但在执行开关打开前，用户启动会被拒绝。推荐“验证 → 核对 → 开启执行 → 发布”，避免发布一个不可执行应用。
