	...
	return func(c *gin.Context) {
		...
		if found {
			if common.GetContextKeyBool(c, constant.ContextKeyTokenModelLimitEnabled) {
				...
				if !limitsOK || !typeOK || !tokenModelLimitAllows(tokenModelLimit, matchName) {
					abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorTokenModelForbidden, map[string]any{"Model": modelRequest.Model}))
					return
				}
			}
			if existing, pinned, _ := constraints.ResolvedPin(); pinned {
				if existing.ChannelId != affinityChannelID {
					abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorInvalidChannelId))
					return
				}
			}
			constraints.AddPin(taskdto.ChannelPin{
				ChannelId: affinityChannelID,
				Source:    taskdto.PinSourceAsset,
				Rank:      taskdto.PinRankAsset,
				RetryMode: taskdto.PinRetrySameChannel,
			})
			assetAffinityFound = true
		}
		if pin, pinned, overridden := constraints.ResolvedPin(); pinned {
