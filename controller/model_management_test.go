...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	if kind == "sqlite" {
		...
		isolatedDSN = "local"
		// InitDB opens its own SQLite handle below. Close the disposable handle
		// used only to allocate the isolated path so Windows can remove the
		// temporary audit database during test cleanup.
