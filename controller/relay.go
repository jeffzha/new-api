...
func executeTaskSubmissionWith(
	...
	stage := "start"
	defer func() {
		if !durable && taskErr != nil && relayInfo.Billing != nil {
	...
	// ── 成功：结算 + 日志 + 插入任务 ──
	// Task persistence and settlement are handled by the single lifecycle below.
	...
	task.PrivateData.Endpoint = result.Endpoint
	...
	if model.IsAgencyDurableUser(relayInfo.UserId) {
		// Durable agency tasks keep the frozen reservation until the provider
		// reaches a terminal state and the billing journal is finalized.
	...
	if task.BillingReconciliationPending && result.ProviderBilling != nil {
