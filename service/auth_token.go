...
type SecurityProofDetails struct {
	Method   string
	IssuedAt time.Time
	ID       string
}
...
func VerifySecurityProof(raw string, identity AuthIdentity, requiredScope string, allowedMethods []string) (string, error) {
	details, err := VerifySecurityProofDetails(raw, identity, requiredScope, allowedMethods)
	if err != nil {
		return "", err
	}
	return details.Method, nil
}

func VerifySecurityProofDetails(raw string, identity AuthIdentity, requiredScope string, allowedMethods []string) (SecurityProofDetails, error) {
	claims, err := parseAuthClaims(raw, securityProofTokenUse, authSigningKey(securityProofTokenUse))
	if err != nil {
		return SecurityProofDetails{}, err
	}
	userID, err := strconv.Atoi(claims.Subject)
	if err != nil || userID != identity.UserID || claims.SessionID != identity.SessionID || claims.UserAuthVersion != identity.UserAuthVersion || claims.SessionVersion != identity.SessionVersion {
		return SecurityProofDetails{}, ErrAuthTokenInvalid
	}
	...
	if !methodAllowed {
		return SecurityProofDetails{}, ErrProofMethod
	}
	if requiredScope != "" {
		found := false
		...
		if !found {
			return SecurityProofDetails{}, ErrProofScope
		}
	}
	return SecurityProofDetails{
