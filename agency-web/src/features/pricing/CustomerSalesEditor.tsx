...
function CustomerSalesMatrix(props: { userId: string | number; username: string; rows: CustomerSalesRow[]; globalSalesBPS: number | null; agencyDefaultSalesBPS: number; minSpreadBPS: number; salesCapBPS: number; onBack: () => void }) {
  ...
  return <section className="customer-pricing-page">
    ...
    <div className="customer-pricing-bulk"><strong>{t("Batch adjust selected models")}</strong><span>{t("Selected")}: {selected.length}</span><input aria-label={t("Adjustment amount")} inputMode="decimal" placeholder="0.1000 / -0.1000" value={adjustment} onChange={(event) => setAdjustment(event.target.value)} /><button type="button" className="secondary" disabled={!selected.length} onClick={applyAdjustment}>{t("Increase/decrease sales prices")}</button><small>{t("Enter a positive value to increase sales prices or a negative value to decrease them. Cost, minimum spread and sales cap still apply.")}</small></div>
    <div className="table-wrap pricing-matrix-wrap customer-pricing-matrix-wrap customer-pricing-page-matrix"><table className="pricing-matrix customer-pricing-matrix"><thead><tr><th><input type="checkbox" aria-label={t("Select visible models")} checked={allVisibleSelected} onChange={toggleVisible} /></th><th>{t("Model name")}</th><th>{t("My cost (coefficient)")}</th><th>{t("Sales price (coefficient)")}</th></tr></thead><tbody>{pageRows.map((row) => <tr key={row.model}><td><input type="checkbox" aria-label={t("Select model") + ": " + row.model} checked={selected.includes(row.model)} onChange={() => toggle(row.model)} /></td><td><strong>{row.model}</strong></td><td><span className="coefficient-readonly">{formatCoefficient(row.agencyCostBPS)}</span></td><td><input aria-label={t("Sales price (coefficient)") + ": " + row.model} inputMode="decimal" placeholder={formatCoefficient(row.inheritedSalesBPS)} value={values[row.model] ?? ""} onChange={(event) => setValues((current) => ({ ...current, [row.model]: event.target.value }))} /><small>{values[row.model] ? t("Customer override") : t("Using inherited sales price {{value}}", { value: formatCoefficient(row.inheritedSalesBPS) })}</small></td></tr>)}</tbody></table></div>
