...
if (Test-ListenPort -Port 3201) {
    ...
    $agencyBackendCommand = "Set-Location -LiteralPath '$rootLiteral'; `$Host.UI.RawUI.WindowTitle = 'Agency Hub Backend :3201'; Remove-Item Env:SQL_DSN -ErrorAction SilentlyContinue; Remove-Item Env:LOG_SQL_DSN -ErrorAction SilentlyContinue; `$env:SQLITE_PATH = '$sqliteLiteral'; `$env:AGENCY_HUB_PORT = '3201'; `$env:AGENCY_HUB_BASE_PATH = '/agency'; `$env:AGENCY_HUB_PUBLIC_BASE_URL = 'http://localhost:3001'; `$env:AGENCY_HUB_PLATFORM_BASE_URL = 'http://localhost:3001'; `$env:AGENCY_HUB_BROWSER_ORIGIN = 'http://localhost:3202'; `$env:AGENCY_HUB_COOKIE_SECURE = 'false'; `$env:AGENCY_HUB_AUTO_MIGRATE = 'true'; `$env:AGENCY_HUB_FACT_PROJECTION_ENABLED = 'true'; `$env:AGENCY_ONBOARDING_ENABLED = 'true'; `$env:AGENCY_HUB_COMMISSION_PROCESSING_ENABLED = 'true'; `$env:AGENCY_HUB_WITHDRAWALS_ENABLED = 'true'; `$env:AGENCY_HUB_COMMAND_ALLOW_LOCAL_SQLITE = 'true'; `$env:AGENCY_HUB_SSO_PUBLIC_KEY_FILE = '$agencySSOPublicLiteral'; `$env:AGENCY_HUB_COMMAND_SERVICE_PUBLIC_KEY_FILE = '$agencyCommandPublicLiteral'; `$env:AGENCY_HUB_COMMAND_SERVICE_PRIVATE_KEY_FILE = '$agencyCommandPrivateLiteral'; `$env:AGENCY_HUB_DELIVERY_KEY_FILE = '$agencyDeliveryLiteral'; `$env:AGENCY_PAYOUT_KEY_FILE = '$agencyPayoutLiteral'; go run ./cmd/agency-hub"
    Start-Process -FilePath 'powershell.exe' `
...
if (Test-ListenPort -Port 3202) {
    ...
    $agencyFrontendCommand = "Set-Location -LiteralPath '$agencyWebLiteral'; `$Host.UI.RawUI.WindowTitle = 'Agency Hub Frontend :3202'; `$env:VITE_AGENCY_PLATFORM_BASE_URL = 'http://localhost:3001'; bun run dev -- --host 0.0.0.0 --port 3202"
    Start-Process -FilePath 'powershell.exe' `
...
Wait-HttpReady -Url 'http://localhost:3202/agency/' -ServiceName 'Agency Hub frontend'
...
Wait-HttpReady -Url 'http://localhost:3001' -ServiceName 'New API frontend'
