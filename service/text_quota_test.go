...
func TestGenerateTextOtherInfoSnapshotsBillingConfiguration(t *testing.T) {
	...
	other := GenerateTextOtherInfo(ctx, relayInfo, 0.5, 1, 4, 0, 0.25, -1, -1).Snapshot()
...
func TestCacheCreationPricingSnapshotDistinguishesFallbackFromConfiguredPrice(t *testing.T) {
	...
	require.Equal(t, false, snapshot["cache_creation_pricing_configured"])
	require.NotContains(t, snapshot, "cache_creation_ratio")
	require.NotContains(t, snapshot, "cache_creation_ratio_5m")
	require.NotContains(t, snapshot, "cache_creation_ratio_1h")

	appendCacheCreationPricing(other, false, 10, 1.25, 1.25, 2)
	snapshot = other.Snapshot()

	require.Equal(t, 1.25, snapshot["cache_creation_ratio"])
	require.Equal(t, 1.25, snapshot["cache_creation_ratio_5m"])
	require.Equal(t, 2.0, snapshot["cache_creation_ratio_1h"])
}
