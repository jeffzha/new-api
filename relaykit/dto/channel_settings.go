...
type ChannelOtherSettings struct {
	...
	DoubaoVideoEndpoints                  *DoubaoVideoEndpointSettings `json:"doubao_video_endpoints,omitempty"`
	...
	// OllamaOpenAIChat routes Ollama chat requests through its OpenAI-compatible endpoint.
...
type DoubaoVideoEndpointSettings struct {
	SubmitPath string `json:"submit_path,omitempty"`
	FetchPath  string `json:"fetch_path,omitempty"`
}

func (s *DoubaoVideoEndpointSettings) Validate() error {
	if s == nil {
		return nil
	}
	if err := validateDoubaoVideoEndpointPath(s.SubmitPath, false); err != nil {
		return fmt.Errorf("doubao_video_endpoints.submit_path: %w", err)
	}
	if err := validateDoubaoVideoEndpointPath(s.FetchPath, true); err != nil {
		return fmt.Errorf("doubao_video_endpoints.fetch_path: %w", err)
	}
	return nil
}

func validateDoubaoVideoEndpointPath(path string, requireTaskID bool) error {
	...
	if parsed.IsAbs() || parsed.Host != "" || parsed.RawQuery != "" || parsed.Fragment != "" {
		return fmt.Errorf("must be a relative path without host, query, or fragment")
	}
	placeholderCount := strings.Count(path, "{task_id}")
	if requireTaskID && placeholderCount != 1 {
		return fmt.Errorf("must contain exactly one {task_id} placeholder")
	}
	if !requireTaskID && placeholderCount != 0 {
		return fmt.Errorf("must not contain a {task_id} placeholder")
	}
	return nil
}
...
const (
	// AdvancedCustomConverterSGLangRerank adapts Jina-style rerank requests
	// to the SGLang rerank endpoint. It is exported because the host module
	// provides the built-in SGLang preset.
...
// SupportsPassThroughBody reports whether the route can forward the original
// request body without protocol conversion.
...
func validateAdvancedCustomConverterPath(index int, incomingPath string, converter string) error {
	...
	case AdvancedCustomConverterSGLangRerank:
