...
func appendCacheCreationPricing(other *model.LogOther, configured bool, cacheWriteTokens int,
	cacheCreationRatio, cacheCreationRatio5m, cacheCreationRatio1h float64) {
	other.SetPublic("cache_creation_pricing_configured", configured)
	if !configured && cacheWriteTokens <= 0 {
		return
	}
	other.SetPublic("cache_creation_ratio", cacheCreationRatio)
	other.SetPublic("cache_creation_ratio_5m", cacheCreationRatio5m)
	other.SetPublic("cache_creation_ratio_1h", cacheCreationRatio1h)
}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	other.SetPublic("image_ratio", relayInfo.PriceData.ImageRatio)
	...
	if otherRatios := relayInfo.PriceData.OtherRatios(); len(otherRatios) > 0 {
		other.SetPublic("other_ratios", otherRatios)
	}
