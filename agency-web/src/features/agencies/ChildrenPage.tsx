...
function HierarchySummary(props: { data: HierarchyResponse }) {
  ...
  return <section className="hierarchy-card" aria-label={t("Agency hierarchy")}>
    ...
    <div className="hierarchy-lanes" role="tree" aria-label={t("Agency hierarchy")}>
      <div className="hierarchy-lane hierarchy-upstream"><span className="hierarchy-lane-label">{t("Upstream agencies")}</span>{[...props.data.parents].reverse().map((item) => <HierarchyNode key={item.id} item={item} label={t("Parent agency")} />)}</div>
      <div className="hierarchy-lane hierarchy-current"><span className="hierarchy-lane-label">{t("Current agency")}</span><HierarchyNode item={props.data.current} label={t("Current agency")} current /></div>
      <div className="hierarchy-lane hierarchy-children"><span className="hierarchy-lane-label">{t("Direct child agencies")}</span><div className="hierarchy-child-grid">{props.data.children.map((item) => <HierarchyNode key={item.id} item={item} label={t("Direct child agency")} child />)}{!props.data.children.length && <p className="hierarchy-empty">{t("No direct child agencies yet.")}</p>}</div></div>
    </div>
...
function ChildDelivery(props: { delivery: PasswordDelivery; onClose: () => void }) {
  const { t } = useTranslation();
  return <Dialog title={t("Child agency login details")} onClose={props.onClose}><p className="notice">{t("Save these details now. The temporary password is shown only once and must be changed at first sign-in.")}</p><div className="credential-grid"><Field label={t("Operator username")}><input readOnly value={props.delivery.operator_username || ""} /></Field><Field label={t("Temporary password")}><input readOnly value={props.delivery.temporary_password || ""} /></Field></div>{props.delivery.invite_url && <Field label={t("Invitation link")}><input readOnly value={props.delivery.invite_url} /></Field>}<div className="credential-delivery-footer"><div>{props.delivery.temporary_password_expires_at && <p>{t("Delivery expires at")}: <Time value={props.delivery.temporary_password_expires_at} /></p>}</div><div className="actions"><button type="button" className="button-icon compact-action" onClick={() => void navigator.clipboard.writeText(props.delivery.temporary_password || "")}><ActionIcon name="save" />{t("Copy password")}</button>{props.delivery.invite_url && <button type="button" className="secondary button-icon compact-action" onClick={() => void navigator.clipboard.writeText(props.delivery.invite_url || "")}><ActionIcon name="save" />{t("Copy invitation link")}</button>}<button className="secondary compact-action" type="button" onClick={props.onClose}>{t("Close")}</button></div></div>{props.delivery.invite_qr_url && <div className="credential-qr"><p>{t("Invitation QR code")}</p><img src={props.delivery.invite_qr_url} alt={t("Invitation QR code")} /></div>}</Dialog>;
}
