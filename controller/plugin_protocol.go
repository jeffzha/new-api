...
func streamTaskPluginProtocol(
	...
	lastStatus := ""
	for {
		// A client disconnect can race with the completion of the in-flight
		// load. Check cancellation before starting another database read so a
		// terminal observation cannot be re-entered after the request is gone.
		if observationContext.Err() != nil || c.Request.Context().Err() != nil {
			return
		}
