...
export const adminApi = {
  ...
  verifyUnifiedAgentStoreListing: (body: import('./contracts').UnifiedAgentStoreListingInput) => apiRequest<import('./contracts').UnifiedAgentStoreListingPreview>('/agent-store/listings/verify', { method: 'POST', body }),
  publishUnifiedAgentStoreListing: (itemId: string, expectedVersion: number, expectedDeploymentVersion: number) => apiRequest<import('./contracts').AgentStoreItem>(`/agent-store/items/${encodeURIComponent(itemId)}/listing/publish`, { method: 'POST', body: { expected_version: expectedVersion, expected_deployment_version: expectedDeploymentVersion } }),
  ...
  saveCustomerApp: (customerId: number, selector: string, body: import('./contracts').AppConfigInput) => apiRequest<import('./contracts').AppDraftResult>(`/customers/${customerId}/apps/${encodeURIComponent(selector)}`, { method: 'PUT', body }),
