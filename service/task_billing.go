...
func taskBillingOther(task *model.Task) *model.LogOther {
	...
	if bc := task.PrivateData.BillingContext; bc != nil {
		...
		if bc.AgencyPricing != nil {
			other.SetPublic("agency_sales_bps", bc.AgencyPricing.SalesBPS)
			other.SetPublic("agency_settlement_bps", bc.AgencyPricing.SettlementBPS)
			other.SetPublic("agency_standard_quota", bc.AgencyStandardQuota)
			other.SetPublic("agency_paid_allocated_quota", bc.AgencyPaidAllocatedQuota)
		}
	}
