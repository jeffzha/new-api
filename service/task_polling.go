...
// TaskPollingFetchAtAdaptor allows a task to poll using the endpoint frozen
// at submission time. Implementations that do not provide it retain the
// legacy FetchTask behavior.
type TaskPollingFetchAtAdaptor interface {
	FetchTaskAt(baseURL, key, fetchPath string, task *model.Task, proxy string) (*http.Response, error)
}

// TaskPollingSanitizer removes provider-private identifiers before task data
// is persisted or exposed through task retrieval APIs.
type TaskPollingSanitizer interface {
	SanitizeTaskData(task *model.Task, responseBody []byte) ([]byte, error)
}

type TaskPollingCheckedBillingAdaptor interface {
	AdjustBillingOnCompleteChecked(task *model.Task, taskResult *relaycommon.TaskInfo) (int, *common.QuotaClamp, bool, error)
}
...
func sweepTimedOutTasks(ctx context.Context) {
	...
	for _, task := range tasks {
		if model.IsAgencyDurableUser(task.UserId) {
			// A timeout does not prove provider failure. Keep the reservation and
			// mark the financial journal for operator/provider reconciliation.
			if err := model.MarkAgencyTaskReconcileRequired(task, "task_timeout"); err != nil {
				logger.LogError(ctx, fmt.Sprintf("mark agency timeout reconciliation failed for task %s: %v", task.TaskID, err))
			}
			continue
		}
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	snap := task.Snapshot()
	fetchPath := ""
	if privateData.Endpoint != nil {
		if privateData.Endpoint.BaseURL != "" {
			baseURL = privateData.Endpoint.BaseURL
		}
		fetchPath = privateData.Endpoint.FetchPath
	}
	var resp *http.Response
	var err error
	if fetcher, ok := adaptor.(TaskPollingFetchAtAdaptor); ok && fetchPath != "" {
		resp, err = fetcher.FetchTaskAt(baseURL, key, fetchPath, task, proxy)
	} else {
	...
	if err = common.Unmarshal(responseBody, &responseItems); err == nil && responseItems.IsSuccess() && responseItems.Data.TaskID != "" {
		...
		// Legacy Suno-compatible gateways return an array under data. Keep
		// this parser alongside the rc.37 single-task envelope so existing
		// channels continue to settle correctly.
		var legacy taskdto.TaskResponse[[]taskdto.SunoDataResponse]
		if err = common.Unmarshal(responseBody, &legacy); err == nil && legacy.IsSuccess() && len(legacy.Data) > 0 {
			item := legacy.Data[0]
	...
	if sanitizer, ok := adaptor.(TaskPollingSanitizer); ok {
		if sanitized, sanitizeErr := sanitizer.SanitizeTaskData(task, responseBody); sanitizeErr != nil {
			return recordPollFailure(ctx, adaptor, task, snap.Status, pollClassHookError, resp.StatusCode, sanitizeErr.Error())
		} else {
	...
	if shouldFinalizeBilling && managed {
		if err := completeAgencyTaskBilling(ctx, task, task.Status, int64(taskResult.TotalTokens)); err != nil {
			return err
		}
		return nil
	}
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	...
	// Prefer the checked provider billing hook so frozen provider pricing and
	// quota saturation markers survive settlement.
	if checked, ok := adaptor.(TaskPollingCheckedBillingAdaptor); ok {
		actualQuota, clamp, handled, err := checked.AdjustBillingOnCompleteChecked(task, taskResult)
		if err != nil {
			logger.LogWarn(ctx, fmt.Sprintf("task %s checked billing failed; retaining pre-consume: %v", task.TaskID, err))
			return true
		}
		if handled {
...
func recordPollFailure(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, fromStatus model.TaskStatus, class string, statusCode int, detail string) error {
	if model.IsAgencyDurableUser(task.UserId) && (class == pollClassTransport || class == pollClassHookError || class == pollClassTransient || class == pollClassAuth || class == pollClassNotFound) {
		// These responses do not establish a terminal provider outcome. Never
		// refund a durable Agency reservation on an ambiguous poll result.
		if err := model.MarkAgencyTaskReconcileRequired(task, "provider_response_unknown"); err != nil {
			return err
		}
		return fmt.Errorf("agency task poll outcome unknown: %s", class)
	}
...
func failTaskFromPoll(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, fromStatus model.TaskStatus, reason string) error {
	...
	if model.IsAgencyDurableUser(task.UserId) {
		// An explicit terminal failure is safe to finalize, but if settlement
		// itself cannot commit the journal, retain the reservation for retry.
		return completeAgencyTaskBilling(ctx, task, model.TaskStatusFailure, 0)
	}
	billingSettled := settleTaskBillingOnComplete(ctx, adaptor, task, taskResult)
