			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					t.Cleanup(func() {
						...
						if mainDB != nil {
							if connection, err := mainDB.DB(); err == nil {
								_ = connection.Close()
							}
						}
					})
					...
					for range 2 {
						previousConnection, connectionErr := model.DB.DB()
						require.NoError(t, connectionErr)
						require.NoError(t, model.InitDB())
						require.NoError(t, previousConnection.Close())
						require.NoError(t, model.InitLogDB())
					}
					if !upgrade {
						require.NoError(t, model.DB.Create(&model.User{Username: "fresh-owner", Password: "placeholder", AffCode: "fresh-aff"}).Error)
					}
					...
					if upgrade {
						...
						var old model.Log
						require.NoError(t, model.DB.First(&old).Error)
						assert.Equal(t, "historical login", old.Content)
					}
					require.NoError(t, model.UpdateUserAccessToken(1, "matrix-token"))
					require.Error(t, model.DB.Create(&model.User{Username: "duplicate", Password: "placeholder", AffCode: "duplicate-aff", AccessToken: common.GetPointer("matrix-token")}).Error, "PAT uniqueness must survive upgrade")
